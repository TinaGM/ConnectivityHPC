...
i = 70 #time step- change this!
u = 5000 #euclidean distance threshold- change this!
model<- "cluster" #conservation strat- change this!
species = "s" #s or g for habitat specialist or generalist- change this!
replicate<- 1 #model replicate- change this!


XO<- list.files(paste0(model,"/"), pattern =paste(species,".tif$",sep=""))
...
Landis_standmap <- raster(paste0("Standmap100.tif",sep=""))

#Read in roads file
roads <- raster(paste0("roads.tif",sep=""))
...
write.table(maketext, file=paste0("Outputs/nodes_",u,model,species,"yr",i,"Rep_",replicate,".txt"), sep = "\t", row.names = FALSE, col.names = FALSE)
...
write.table(costcomplete, file=paste0("Outputs/distance_",u,model,"yr",i,"Rep_",replicate,".txt"), row.names=F, col.names=F)
...
write.table(results, file=paste0("Outputs/LeastCostTimeStepn",u,model,"yr",i,"Rep_",replicate,".txt"), row.names=T, col.names=T)