...
Ecoregion <- raster(paste("Ecoregion100f.tif",sep=""))
...
for (i in TimestepList) 
  ...
  Year0<-list.files(pattern=(".img$"))
  
  Longleaf_Stack<-raster(Year0[Year0 %in% paste0("bio-", Longleaf,"-",i,".img")])
  Loblolly_Stack<-raster(Year0[Year0 %in% paste0("bio-", Loblolly,"-",i,".img")])
  Pine_Stack<-raster::stack(Year0[Year0 %in% paste0("bio-", Pine,"-",i,".img")])
  Hardwood_Stack<-raster::stack(Year0[Year0 %in% paste0("bio-", Hardwood,"-",i,".img")])
  Total<-raster(Year0[Year0 %in% paste0("bio-TotalBiomass-", i, ".img")])
  ...
  median0 <- raster(Year0[Year0 %in% paste0("AGE-MED-",i,".img")])
  ...
  write.table(maketext, file=paste("nodes_TMgeos1_",i,".txt"), sep = "\t", row.names = FALSE, col.names = FALSE)
  ...
  write.table(costcomplete, file=paste("distance_TMgeos1_",i,".txt"), row.names=F, col.names=F)
  write.csv(costcomplete, file=paste("distancec_TMgeos1_",i,".csv"), row.names=F)
...
write.table(results, file=paste("results.txt"), row.names=T, col.names=T)
