...
XO<- list.files("BiggestRun",pattern = "^geo(.*)tif$")
ManyRunsStack<-raster::stack(paste0("BiggestRun/",XO))
...
write.table(maketext, file=paste0("Outputs/nodes_TMgeog1_",i,".txt"), sep = "\t", row.names = FALSE, col.names = FALSE)
...
write.table(costcomplete, file=paste0("Outputs/distance_TMgeog1_",i,".txt"), row.names=F, col.names=F)
write.csv(costcomplete, file=paste0("Outputs/distancec_TMgeog1_",i,".csv"), row.names=F)
...
write.table(results, file=paste0("Outputs/LeastCostTimeStepg1_",i,"_results.txt"), row.names=T, col.names=T)