...
for (i in TimestepList) {
  ...
  Year0<-list.files(Drive,pattern=paste0("\\-",i,".img$"))
  
  Longleaf_Stack<-raster(paste0(Drive,(Year0[Year0 %in% paste0("bio-", Longleaf,"-",i,".img")])))
  Loblolly_Stack<-raster(paste0(Drive,(Year0[Year0 %in% paste0("bio-", Loblolly,"-",i,".img")])))
  Pine_Stack<-raster::stack(paste0(Drive,(Year0[Year0 %in% paste0("bio-", Pine,"-",i,".img")])))
  Hardwood_Stack<-raster::stack(paste0(Drive,(Year0[Year0 %in% paste0("bio-", Hardwood,"-",i,".img")])))
  Total<-raster(paste0(Drive,(Year0[Year0 %in% paste0("bio-TotalBiomass-", i, ".img")])))
  ...
  median0 <- raster(paste0(Drive,(Year0[Year0 %in% paste0("AGE-MED-",i,".img")])))
  ...
  LU0 <- raster(paste0(Drive, "NLCD100.tif"))
  ...
  write.table(maketext, file=paste(Drive, "nodes_TMgeos1_",i,".txt"), sep = "\t", row.names = FALSE, col.names = FALSE)
  ...
  write.table(costcomplete, file=paste(Drive, "distance_TMgeos1_",i,".txt"), row.names=F, col.names=F)
  write.csv(costcomplete, file=paste(Drive, "distancec_TMgeos1_",i,".csv"), row.names=F)
...
results <- data.frame(TimestepList, nodes, links, avgnode, totnode, avgLCP, avgENN, density, transitivity)
write.table(results, file=paste(Drive, "results.txt"), row.names=T, col.names=T)


#Run conefor command line
setwd(Drive)
shell("coneforWin64.exe -nodeFile nodes_TMgeos1_ -conFile distance_TMgeos1_ -t dist notall -* -confProb 50000 0.5 -PC onlyoverall")
#shell("coneforWin64.exe -nodeFile Nodes1.txt -conFile Distances1.txt -t dist notall -confProb 220000 0.5 -PC onlyoverall"
